...
relation {
  ...
  streaming_inventory               = "true|false"
  streaming_inventory_output_bucket = "string"
  streaming_inventory_table         = "string"
}
```
...
--
#### streaming\_inventory = "true | false"

Optionally create an AWS based system to track S3 inventory on the `storage_bucket` for a given relation. ConvergDB will create the following:

* S3 event that triggers a lambda function whenever a new file arrives in the bucket.
* Lambda function that parses the S3 event object to extract only the fields we want.
* Kinesis Firehose to deliver the data to another S3 bucket (admin bucket) in JSON format.
* Athena table build on top of the JSON inventory output for use in ETL jobs.

If this field is set to `true`, the inventory table will be used by the ETL job. Files that existed in the source bucket previous to building the streaming\_inventory objects in AWS will not be in the inventory. In the case of migrating your historical data, you should use the nightly S3 inventory generated by AWS. Once you have overlap between the AWS generated inventory report and the ConvergDB, meaning that the streaming inventory has been running since the last full inventory reports, you can switch from the S3 inventory to the streaming_inventory.

Note that the AWS user running ConvergDB will need the appropriate IAM permissions to attach an event notification to the bucket.

--
#### streaming\_inventory\_output\_bucket = "bucket/prefix"

Defaults to the admin bucket. You can override it with this attribute, but we advise against it.

--
#### streaming\_inventory\_table = "database.table_name"

Defaults to an automatically generated table. You can override it with this attribute, but we advise against it.
...
athena "environment_name" {
    ...
    relations {
        relation {
            ...
            # optional
            use_inventory     = "true"
            inventory_source  = "default|s3|streaming|api"
            domain_name       = "string"
            schema_name       = "string"
            relation_name     = "string"
            source_relation_prefix = "string"
...
--
#### dsd = "domain.schema.derived\_relation\_name"
...
--
#### inventory\_source  = "default|s3|streaming|api"

Specifies which inventory source to use from the following choices:

* `default` - Will use whatever the source relation indicates.
* `api` - Force AWS S3 API based inventory.
* `s3` - Requires the source relation to have an `inventory_table` specified.
* `streaming` - Requires the source relation to have `streaming_inventory == "true"`
